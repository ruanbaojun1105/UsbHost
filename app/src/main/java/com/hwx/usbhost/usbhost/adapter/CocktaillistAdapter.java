...
public class CocktaillistAdapter extends BaseQuickAdapter<Cocktail> {
    boolean isEdit;
    boolean isForResult;
    int position;
    ...
    protected void convert(BaseViewHolder baseViewHolder, Cocktail cocktail) {
        ...
        if (!TextUtils.isEmpty(cocktail.getPreviewImage())) {
            if (TextUtils.isDigitsOnly(cocktail.getPreviewImage())) {
                DrawableUtil.displayImage(baseViewHolder.getConvertView().getContext(), baseViewHolder.getView(R.id.item_img), ImageListAdapter.getImageTypeUrl(Integer.parseInt(cocktail.getPreviewImage())));
            } else {
                DrawableUtil.displayImage(baseViewHolder.getConvertView().getContext(), baseViewHolder.getView(R.id.item_img), cocktail.getPreviewImage());
            }
        }
        baseViewHolder.setOnClickListener(R.id.item_img, view -> {
            if (isEdit){
        ...
        baseViewHolder.setOnLongClickListener(R.id.item_img, view -> {
            ...
            if (!isEdit)
                return true;
            if (!AppConfig.getInstance().getBoolean("peifangjinzhi",false)) {
                DialogUtil.showListDialog(baseViewHolder.getConvertView().getContext(), null, new String[]{"delete"}, which -> {
        ...
        baseViewHolder.setOnClickListener(R.id.item_delect, view -> {
            if (!AppConfig.getInstance().getBoolean("peifangjinzhi",false)){
                CocktailManager.getCocktailManager().deleteCocktail(getItem(baseViewHolder.getAdapterPosition()-1));
                remove(baseViewHolder.getAdapterPosition()-1);
            }else
        ...
        baseViewHolder.setOnClickListener(R.id.item_other, view -> {
            if (!AppConfig.getInstance().getBoolean("peifangjinzhi",false)){
                UpdateCocktailActivity.openIntance(baseViewHolder.getConvertView().getContext(), cocktail);
            }else
                Toast.makeText(baseViewHolder.getConvertView().getContext(),R.string.dvfdsaioi+"\n"+mContext.getString(R.string.vicicdkr),Toast.LENGTH_SHORT).show();
